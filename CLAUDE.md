...
- Usable without an account (see Guest access below)
- **Ideas panel** ("Ideas" tab, right column, alongside "Analyze"): for the position currently on the board, shows three visually distinct sections — your stats (computed, e.g. "you play 8...Bd7 (29%), main line is 8...Re8"), structural facts (computed pawn-structure/open-file/outpost classification, no prose), and plans (2-3 bullets per side, LLM-synthesized but grounded in the structural facts plus a retrieved Wikibooks excerpt, with a source citation) — never blended into one narrative, since the three sources carry different reliability. See `backend/motifs.py` (structural classifier), `backend/theory_corpus.py` + `backend/scripts/ingest_theory.py` (Wikibooks CC BY-SA retrieval corpus, run manually), `backend/motif_cache.py` (LLM synthesis + Postgres cache keyed by FEN, shared across all owners), `backend/routers/motifs.py` (`GET /motifs`), and `frontend/src/components/MotifPanel.jsx`. Plans are only generated by the background precompute after "Rebuild from Games" (`motif_cache.precompute_for_owner_tree`); `GET /motifs` is read-only against the cache and never calls the LLM, so positions outside the rebuilt tree (e.g. manually-added lines, Analytics deep-links) show stats + structure with no plans until the next rebuild. Precompute only runs for logged-in accounts — a guest rebuild never calls the LLM (guests still see plans already cached for a position, since the cache is shared). LLM calls are also hard-capped over a rolling 24h window via `motif_generation_log` — per account (`MOTIF_DAILY_CAP_PER_USER`, default 200) and globally (`MOTIF_DAILY_CAP_GLOBAL`, default 1000); once hit, precompute stops and remaining positions are picked up by a later rebuild. Only non-empty plans are cached, so a missing key or failed call is retried rather than stuck empty. This bounds LLM spend. `GET /motifs` is also rate-limited per owner (in-process, 30 req/min; fine for the single-instance Railway deploy). Requires `ANTHROPIC_API_KEY` in backend `.env` — the plans section silently omits itself (stats/structure still show) if unset.
